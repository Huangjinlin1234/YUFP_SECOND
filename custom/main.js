...
(function (window, yufp) {
  ...
  yufp.require.use(libs).done(function () {
    ...
    var hashFn = function () {
      ...
      if (hash && hash.slice(sIndex + 1)) {
        route = hash.slice(sIndex + 1);
        if (route.startsWith('preview')) {
          var rPath = data.path;
          rPath = rPath.replaceAll('##', '/');
          var r = {};
          route = rPath.replaceAll('/', '-').replace('-', '');
          r[route] = {};
          r[route].html = `${rPath}.html`;
          if (data.jsFlag == 'true') {
            r[route].js = `${rPath}.js`;
          }
          if (data.cssFlag == 'true') {
            r[route].css = `${rPath}.css`;
          }
          yufp.router.addRouteTable(r);
        }
      }
