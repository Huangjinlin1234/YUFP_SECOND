...
(function (yufp, window, factory) {
  ...
  var setUserInfo = function () {
    ...
    // 按长度截角色信息
    var roleStr = role.join(',');
    if (roleStr && roleStr.length > maxlength) {
      document.getElementById('userRole').innerHTML += '(' + roleStr.substring(0, maxlength) + '......)';
      document.getElementById('userRole').title = roleStr;
    } else {
      document.getElementById('userRole').innerHTML += '(' + roleStr + ')';
    }
    // 按个数截角色信息
    //        if(role && role.length > 2) {
    //        	var subRole = [role[0], role[1]];
    //        	document.getElementById('userRole').innerHTML += '(' + subRole.join(',') + ',......)';
    //        } else {
  ...
  Frame.prototype.removeTab = function (key) {
    var _this = this;
    key = key || $('#' + _this.tabsTargetId + '>div>a[class=ck]').attr('data-key');
