...
(function (yufp, window, factory) {
  ...
  function LocalSession () {
    var _settings = {
      settings: {
        ...
        ctrlJsonRoot: 'ctrls', // 控制点返回数据节点,如：'rows.ctrls'，控制点数据，查询需按菜单ID、功能ID排序返回
  ...
  /**
     ...
     * @param obj 待获取对象
     * @param ns namespace，如：'json.rows'
     * @returns {*}
     */
  ...
  LocalSession.prototype.loadUserSession = function (callback) {
    ...
    yufp.service.request({
      ...
      method: 'POST',
      async: false,
      callback: function (code, message, rows) {
        var rows = getObjectKey(rows, _this.settings.userJsonRoot);
        if (code == '0' && rows) {
  ...
  LocalSession.prototype.loadMenus = function (callback) {
    ...
    yufp.service.request({
      ...
      method: 'POST',
      callback: function (code, message, rows) {
        var menuData = getObjectKey(rows.rows, _this.settings.menuJsonRoot);
        if (code == '0' && menuData) {
          _this.processMenus(menuData);
          var ctrlData = getObjectKey(rows.rows, _this.settings.ctrlJsonRoot);
          if (ctrlData) {
  ...
  /**
     ...
     * @param rows
     */
  LocalSession.prototype.processMenus = function (rows) {
    var _this = this, mm = _this.settings.menuMapping;
    _this.orginalMenus = JSON.parse(JSON.stringify(rows));
    ...
    for (var i = 0, len = rows.length; i < len; i++) {
      ...
      if ('' + obj.mPid == '') {
            	obj.mPid = _this.settings.menuRootPid;
      }
  ...
  LocalSession.prototype.loadCtrls = function (callback) {
    ...
    yufp.service.request({
      ...
      method: 'get',
      callback: function (code, message, rows) {
        var rows = getObjectKey(rows, _this.settings.ctrlJsonRoot);
        if (code == '0' && rows) {
          _this.processCtrls(rows);
        }
        typeof callback === 'function' ? callback.call(_this) : '';
      }
    });
  ...
  /**
     ...
     * @param rows
     */
  LocalSession.prototype.processCtrls = function (rows) {
    if (!rows || rows.length < 1) {
      return;
    }
    var _this = this, mm = _this.settings.ctrlMapping;
    _this.orginalCtrls = JSON.parse(JSON.stringify(rows));
