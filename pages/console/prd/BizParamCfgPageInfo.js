      ...
      data: function () {
        ...
        return {
          ...
          tableColumns: [
            ...
            { label: '授信总限额（万）',
              prop: 'crdLmtAmt', /* type:'num',
                                formatter : _self.toPercent,*/ sortable: true,
              resizable: true },
          ...
          updateFields: [{
            columnCount: 2,
            fields: [
                	{ field: 'forbidIndusArr',
                label: '禁入行业',
                hidden: false,
                type: 'custom',
                params: _self,
                	  value: [],
                is: 'biz-param-cfg-selector'
              },
                	 { field: 'crdExpiryDt',
                label: '征信查询有效期（天）',
                rules: [
                ...
                rules: [
                  ...
                  { validator: yufp.validator.number, message: '数字', trigger: 'blur'}
                	   ...
                	   { field: 'chkRptCrtPeriod',
                label: '检查报告生成周期（月）',
                rules: [
                	    {required: true, message: '必填项', trigger: 'blur'},
                  { validator: yufp.validator.number, message: '数字', trigger: 'blur'}
                ]},
                	 { field: 'regChkCrtPeriod',
                label: '定期检查任务生成周期（月）',
                rules: [
                	   {required: true, message: '必填项', trigger: 'blur'},
                  { validator: yufp.validator.number, message: '数字', trigger: 'blur'}
                ]},
                	 { field: 'crdOrgUpperLmt',
                label: '授信机构数量上限（家）',
                rules: [
                	   {required: true, message: '必填项', trigger: 'blur'},
                  { validator: yufp.validator.number, type: 'number', message: '数字', trigger: 'blur'}
              ...
              { field: 'amtIntervalCount',
                label: '提额间隔天数/提额批复有效期（天）',
                rules: [
                  {required: true, message: '必填项', trigger: 'blur'},
                  {validator: yufp.validator.number, type: 'number', message: '数字', trigger: 'blur'}
                ]},
